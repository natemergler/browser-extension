...
The default development settings in `settings/chrome-dev.json` provide:
...
1. `pnpm install` installs the Hypothesis client and build toolchain.
2. `pnpm build` copies client files from `node_modules/hypothesis/build/` to `build/client/`.
3. The extension loads the client from the local build directory.
...
- `apiUrl` - Base URL for API requests (points at the RabbitTrail API)
- `authDomain` - Hostname that issues authentication credentials
- `bouncerUrl` - Base URL used for direct-link resolution
- `serviceUrl` - Base URL for the main web application

> **Note:** Production and staging JSON files contain placeholder OAuth client IDs and Sentry DSNs. Replace them with real credentials before packaging builds.

## Testing

### Test Structure

- A legacy Vitest suite lives in `tests/` and runs against the bundled background modules
- Background script tests cover injection logic, badge updates, and Chrome API interactions using mocked Chrome APIs
- The test bundle is produced with Rollup before executing in Vitest's headless Chromium environment
...
The command generates `build/tests.bundle.js`, launches Playwright's Chromium runner, and reports results using the summary reporter.
...
├── pdfjs-init.js            # PDF viewer bootstrap
├── sidebar-app.html.mustache# Sidebar host template
├── unload-client.js         # Cleanup script
└── vendor/
   └── pdfjs/               # Bundled PDF.js viewer
```
...
2. Run `pnpm dev` for development rebuilds
3. Test changes in Chrome with the unpacked extension
4. `pnpm test` currently only prints a reminder that tests are disabled
